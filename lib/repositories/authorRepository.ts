...
export async function listAuthors(params: ListParams): Promise<ListResult<any>> {
  ...
  if (params.q)
    query = query.or(`name.ilike.%${params.q}%,email.ilike.%${params.q}%,designation.ilike.%${params.q}%`);
...
export async function createAuthor(values: {
  name: string;
