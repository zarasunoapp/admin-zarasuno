...
const TABLE = "publisher_countries";

export async function listPublisherCountries(params: ListParams): Promise<ListResult<any>> {
...
export async function createPublisherCountry(values: { name: string }) {
  const db = createSupabaseAdminClient();
...
export async function updatePublisherCountry(id: string, values: { name: string }) {
  const db = createSupabaseAdminClient();
...
export async function deletePublisherCountry(id: string) {
  const db = createSupabaseAdminClient();
  const { error } = await db.from(TABLE).delete().eq("id", id);
  if (error) throw new Error(error.message);
}

export async function bulkInsertPublisherCountries(rows: { name: string }[]) {
  const db = createSupabaseAdminClient();
...
export async function listAllPublisherCountries() {
  const db = createSupabaseAdminClient();
